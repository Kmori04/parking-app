...
<head>
  ...
  <title>Motors</title>
  @vite(['resources/css/availability_motor.css','resources/js/app.js'])
</head>
<body>
  <header>
  ...
  <main>
    <h1>MOTORCYCLE PARKING SLOTS</h1>

    @php
      // Sort like F1, F2, F10…
      $sorted = $MotorcycleParkingStatusData->sortBy(function($r){
      ...
      // Quick lookup by slot number for the map pins
      $byNumber = $sorted->keyBy('Slot_number');

      // Only rotate groups listed here (text stays upright via CSS). C removed.
      $rotatedGroups = []; // ['C'] -> removed
              ...
              @foreach($available as $s)
                <tr data-slot-id="{{ $s->Slot_id }}" data-slot-number="{{ $s->Slot_number }}">
                  ...
                  <td class="action-cell">
                    <form method="POST" action="{{ route('availability.motors.update', $s->Slot_id) }}" class="status-form">
                      @csrf
        ...
        <div class="campus">
          ...
          @foreach($slotCoords as $slot => [$x,$y])
            @php
              ...
              // Only F is interactive; others (C, D, A, B, E, G) are "Others" (white)
              $isInteractive = $group === 'F';
  ...
  <script>
    (function(){
      ...
      async function updateStatus(slotId, status){
        const res = await fetch(`{{ url('/availability/motors') }}/${slotId}/status`,{
          method:'POST',
